...
| OLED 显示 | SSD1306 128x64，显示音量、进度条、时间、状态 |
| 按键调音量 | 短按 ±1%，长按连续调（无极调节） |
| 播放/暂停 | GPIO 0（BOOT 键）切换暂停/恢复 |
...
### 坑 7：音频流播放卡顿与重放（"一个字唱两遍"）

**症状：** WiFi 网络流媒体播放时偶尔卡顿，甚至出现"把一个字/音节重放两遍"的现象。

**原因链：**

1. **WiFi 省电模式导致延迟尖峰** — ESP-IDF 默认开启 WiFi Power Save，会周期性进入省电状态，导致 TCP 数据到达延迟 100ms+，超过 I2S DMA 缓冲区大小，缓冲区欠载（underrun）。

2. **`auto_clear = false` 导致欠载时重放旧数据** — I2S DMA 配置中 `auto_clear = false` 意味着 DMA 传输完一个描述符后不清除其数据。当缓冲区欠载时，DMA 硬件会重新读取未被清除的旧描述符，把刚播过的音频再播一遍——这就是"唱两遍"的根因。

3. **DMA 缓冲区偏小** — 原始配置 8 个描述符 × 480 帧 ≈ 87ms（44.1kHz 立体声），对 WiFi 网络抖动来说容错空间太小。

4. **解码循环中加 `vTaskDelay(1)` 位置不当** — 放在内层解码循环里（每个解码帧后延迟 1ms），一个 HTTP 块可能触发 20-100 次解码，累积延迟 20-100ms，进一步加速缓冲区消耗。

**最终解决方案（按重要性排序）：**

| 修改 | 位置 | 作用 |
|------|------|------|
| `esp_wifi_set_ps(WIFI_PS_NONE)` | 连上 WiFi 后调用 | **最关键**：消除 WiFi 省电导致的 100ms+ 延迟尖峰 |
| `auto_clear = true` | I2S `chan_cfg` | 欠载时输出静音而不是重放旧数据 |
| `dma_desc_num = 16` | I2S `chan_cfg` | 缓冲区翻倍到 ~174ms，容错更强 |
| 不在解码循环里加延迟 | — | 让 `i2s_channel_write(portMAX_DELAY)` 自然阻塞提供反压即可 |

**教训：**
1. ESP32 音频流播放首先要关闭 WiFi 省电模式，这是最隐蔽但影响最大的因素
2. `auto_clear = true` 比 `false` 更安全——短暂无声比重复播放更不引人注意
3. 不要在热路径（每个解码帧）里加人为延迟；I2S 写入本身在缓冲区满时会自然阻塞并 yield CPU
4. 减少 CPU 频率到 160MHz 在某些情况下可能改善音频质量（高频可能导致总线竞争），可以作为备选方案

### 坑 8：SSD1306 字符只显示一半（16 个字符只看到 8 个）

**症状：** 屏幕每行只显示前面 ~8 个字符，后面的字符被屏幕边框"挡住"了。

**原因：** SSD1306 初始化设置了 Horizontal Addressing Mode（`0x20, 0x00`），但所有写入函数（`display_set_line`、`display_clear`、`display_set_volume`）都使用 Page Addressing Mode 的列地址命令（`0x00`/`0x10`）。两种模式混用导致列地址设置行为不正确。同时缺少 `0x21`（列地址范围）和 `0x22`（页地址范围）命令，某些 SSD1306 兼容芯片上电默认值可能不是 0-127，导致可见区域被截断。

**修复：** 将 `0x20, 0x00`（Horizontal）改为 `0x20, 0x02`（Page），与所有写入函数保持一致。同时添加 `0x21, 0, 127` 和 `0x22, 0, 7` 显式设置地址范围。

**教训：** SSD1306 的寻址模式必须与写入时使用的命令一致。Page Mode 用 `0xB0+page` + `0x00`/`0x10`，Horizontal Mode 用 `0x21`/`0x22` 设定范围后自动递增。混用会导致不确定行为。
